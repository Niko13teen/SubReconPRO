                ...
                for payload in map(lambda line: line.rstrip("\n"), word):  
                    try:  
                        ...
                        if response:  
                            ...
                            title: str = response.html.find("title", first=True)  
                            print(  
                                f"[+] Found: https://{payload}.{self} | {Fore.CYAN}{ip_addr}{Fore.GREEN} | Title: {Fore.WHITE}{title.text if title else None} {Fore.GREEN}| Status: {Fore.CYAN}{response.status_code}{Fore.GREEN} | PageSize: {Fore.CYAN}{len(response.text) / 1024}{Fore.GREEN}"  
                            )  
                    except Exception as error:  
                        #print(f"{Fore.CYAN}DEBUG: {payload}.{self} - Not Found!") 
                        pass  
